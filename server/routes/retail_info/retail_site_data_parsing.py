...
def getSiteInfo(start, end):
  directory = "outputs/" + "rows" + str(start) + "-" + str(end) + ".js"
  f = open(directory, "a")

  for i in range(start, end+1):
    row_value = site_info.loc[i-2] # get row based on index number, first site starts at index 0 (current row - 2)
  ...
  for i in range(start, end+1):
    ...
    f.write(f"""await addOrUpdateSite(site{siteID});\n""")
